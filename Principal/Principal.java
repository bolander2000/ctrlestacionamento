...
public class Principal {
		...
		// criar o carro e cadastra-lo no vetor na posicao correta
                ...
                menu1 = scanner.nextInt();
                    ...
                    if (menu1 <= marcas.get(menu-1).getSizeModelo()){
                        //System.out.println("O modelo do carro é:" + marcas.get(menu-1) +" "+ marcas.get(menu-1).getNomeModelo(menu1-1));
		...
		// logica para calcular preco do estacionamento e coloca-lo no historico
                ...
                switch(menu){
                    ...
                    /*case 2:{
                        ...
                        while(vagas[i].getPlaca() != placa){
                            i++;                                                   
                        }
                        if (vagas[i].getPlaca() == placa){
                            vagas[i].setSaida(LocalDateTime.now());
                            LocalDateTime from = vagas[i].getEntrada();
                            LocalDateTime to = vagas[i].getSaida();
                            Duration duration = Duration.between(from, to);
                            System.out.println("Tempo no estacionamento: "+ duration.toMinutes() + " minutos");
	...
	// outros m�todos static conforme especificacao do trabalho e necessidades de implementacao
        ...
        // logica para calcular preco do estacionamento e coloca-lo no historico
        //public static boolean isSameDay(LocalDateTime timestamp, LocalDate timestampToCompare){
